...
let payoneer = document.getElementById('payoneer');
...
activities.addEventListener('click', (event) => {
  ...
  DisplayCost.innerHTML = "Total Cost: Rs" + cost;
  cost = 0;
  validateCheckbox();
});
...
payment.addEventListener('click', () => {
  if(payment.value === 'credit card') {
    ...
    let ccZipValid = true;
    let ccCVVValid = true;
  } else if(payment.value === 'payoneer') {
    removeRequired();
    paypal.style.display = 'none';
    creditCard.style.display = 'none';
    payoneer.style.display = defaultDisplay;
    let ccNumValid = true;
    let ccZipValid = true;
    let ccCVVValid = true;
  };
